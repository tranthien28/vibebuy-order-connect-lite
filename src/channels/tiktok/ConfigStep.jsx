      ...
      <div className={`flex items-center justify-between p-4 rounded-3xl border transition-all ${isActive ? 'bg-slate-100 border-slate-200' : 'bg-gray-50 border-gray-100 opacity-60'}`}>
        <div>
          <h3 className="text-sm font-black text-gray-900 flex items-center gap-2">
            TikTok Connection
            {isActive && <div className="w-1.5 h-1.5 rounded-full bg-black animate-pulse" />}
          </h3>
          <p className="text-[10px] text-gray-500 font-bold uppercase mt-0.5">{isActive ? 'Profile link active' : 'Channel offline'}</p>
        </div>
      ...
      <div className="p-6 bg-white rounded-3xl border border-gray-100 shadow-sm space-y-4">
        ...
        <div className="relative group">
           <input
             ...
             className="w-full h-14 px-5 bg-gray-50 border-2 border-transparent rounded-2xl text-lg font-bold group-hover:bg-gray-100 focus:bg-white focus:border-black transition-all outline-none"
           />
        </div>
        <p className="text-[10px] text-gray-400 font-medium italic">* Enter your TikTok username with or without @.</p>
      </div>
