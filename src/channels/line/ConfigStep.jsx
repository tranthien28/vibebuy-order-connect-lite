...
const ConfigStep = ({ channel, settings, updateSetting, onNavigate }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className={`flex items-center justify-between p-4 rounded-3xl border transition-all ${isActive ? 'bg-green-50/50 border-green-100' : 'bg-gray-50 border-gray-100 opacity-60'}`}>
        <div>
          <h3 className="text-sm font-black text-gray-900 flex items-center gap-2">
            Line Connection
            {isActive && <div className="w-1.5 h-1.5 rounded-full bg-[#00C300] animate-pulse" />}
          </h3>
      ...
      <div className="p-6 bg-white rounded-3xl border border-gray-100 shadow-sm space-y-4">
        ...
        <div className="relative group">
           <input
             type="text"
             value={get('id')}
             onChange={e => updateSetting(`${prefix}id`, e.target.value)}
             placeholder="your_line_id"
             className="w-full h-14 px-5 bg-gray-50 border-2 border-transparent rounded-2xl text-lg font-bold group-hover:bg-gray-100 focus:bg-white focus:border-green-500 transition-all outline-none"
           />
        </div>
        <p className="text-[10px] text-gray-400 font-medium italic">* Enter your Line ID or Page Slug.</p>
      </div>
