...
const ConfigStep = ({ channel, settings, updateSetting, onNavigate }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      {/* Activation Status */}
      <div className={`flex items-center justify-between p-4 rounded-3xl border transition-all ${isActive ? 'bg-purple-50/50 border-purple-100 shadow-sm' : 'bg-gray-50 border-gray-100 opacity-60'}`}>
        <div>
      ...
      <div className="p-6 bg-white rounded-3xl border border-gray-100 shadow-sm space-y-4">
        ...
        <div className="relative group">
           <input
             ...
             placeholder="10293xxxxxxx"
             className="w-full h-14 px-5 bg-gray-50 border-2 border-transparent rounded-2xl text-lg font-bold group-hover:bg-gray-100 focus:bg-white focus:border-purple-500 transition-all outline-none"
           />
        </div>
        <p className="text-[10px] text-gray-400 font-medium italic">
           * Use your Page ID or custom URL username.
        </p>
      </div>
